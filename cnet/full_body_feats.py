...
from .feature_cnet import Featmap_Model
...
class adapt_net():
    ...
    def __init__(self, config, 
        ...
        if self.config.use_features:
            self.config.ckpt_name = 'feats' + self.config.ckpt_name
        ...
        if R:
            ...
            self.config.cnet_train_epochs = 1 if not eps else eps #6
            ...
            net_num_stages = 1 if not num_stages else num_stages
            net_lin_size = 512 if not lin_size else lin_size
            self.config.cnet_train_epochs = 10 if not eps else eps
        ...
        if self.config.use_features:
            self.cnet = Featmap_Model(linear_size=net_lin_size, num_stages=net_num_stages, p_dropout=0.3, 
        ...
        # Training
        self.config.train_split = 1.0 # 1.0
    ...
    def __call__(self, pose_in, featmaps_in, ret_corr_idxs=False):
        out_pred = pose_in.clone().detach()
        if self.config.use_cnet_energy and self.R == False:
            E_in = self._energy(pose_in)
            if self.config.energy_lower_thresh:
                ...
                dont_corr_idxs = torch.zeros(pose_in.shape[0]).bool().to(self.device)

            # init step sizes
            step_sizes = torch.ones_like(pose_in[:,:1,0]) * self.config.corr_step_size
            ...
            if self.config.energy_scaled_corr:
                ...
                step_sizes *= torch.clamp(1.0 - (E_in - 800)/(800),
                                          min=0.0, max=10.0).reshape(-1,1)
        else:
            dont_corr_idxs = torch.zeros(pose_in.shape[0]).bool().to(self.device)
            step_sizes = torch.ones_like(pose_in[:,:1,0]) * self.config.corr_step_size
        ...
        if corr_idxs.sum() == 0:
            if ret_corr_idxs:
                return pose_in, corr_idxs
            else:
                return pose_in

        # rotate poses to zero orientation
        if self.config.zero_orientation:
            pose_in, rot, i_rot = self._zero_orientation(pose_in[corr_idxs])

        # correct as required
        for i in range(self.config.corr_steps):
            corr_pred = self._corr(pose_in, featmaps_in, step_sizes=step_sizes)
            pose_in = corr_pred
    ...
    def _corr(self, in_pose, in_featmaps, step_sizes):
        '''
        Correct the input poses
        '''
        kpt_inp = in_pose[:,self.in_kpts].flatten(1)
        fm_inp = in_featmaps.flatten(1)
        inp = [kpt_inp, fm_inp]
    ...
    def _load_data_files(self, load_AMASS=False):
        ...
        if load_AMASS:
            ...
            for i, (trainset_path, backbone_scale, data_lim) in enumerate(zip(self.config.cnet_trainset_paths,
                ...
                # DEBUG ----
                if self.config.use_features:
                    # only choose from the samples with extracted feature maps
                    # first, crawl the feature map directory to get the list of samples with feature maps
                    dataset_name = trainset_path.split('/')[-2]
                    feature_map_dir = f'/data/lbidulka/adapt_3d/{dataset_name}/feature_maps/' + 'mmlab_cliff_train'
                    feature_map_files = os.listdir(feature_map_dir)
                    feature_map_ids = [int(f.split('.')[0]) for f in feature_map_files]
                    
                    # next, 
                    data_subset_ids = np.random.choice(feature_map_ids, len(feature_map_ids), replace=False)  
                    # only take samples with ids == subset
                    subset_idxs = np.where(np.isin(data[2,:,0].numpy(), data_subset_ids))[0]
                    data = data[:, subset_idxs, :]
                    self.config.train_data_ids.append(subset_idxs)
                elif data_lim is not None:
                # ----------
                # if data_lim is not None:
                    data_subset_ids = np.random.choice(data.shape[1], 
                                                        min(data_lim, data.shape[1]), 
                                                        replace=False)
                    # get random subset of data
                    data = data[:, data_subset_ids, :]
                    self.config.train_data_ids.append(data_subset_ids)
                ...
                if self.config.use_features:
                    ...
                    trainset_idx = self.config.trainsets.index(trainset_path.split('.')[0].split('/')[-2])
                    data = torch.cat([data, torch.ones_like(data[:1])*trainset_idx], axis=0)
                    # add backbone index to data
                    backbone_idx = self.config.train_backbones.index(trainset_path.split('.')[0].split('/')[-1].split('_')[1])
                    data = torch.cat([data, torch.ones_like(data[:1])*backbone_idx], axis=0)
                ...
                if IS_VAL:
                    ...
                    if len(self.config.val_sets) == 0:
                        ...
                        data_t, data_v = data[:, :len_train, :], data[:, len_train:, :]
                        data_train.append(data_t)   # TEMP: don't include image paths, if they are present
                        data_val.append(data_v)
                    else:
                        data_train.append(data)   # TEMP: don't include image paths, if they are present
    ...
    def train(self, pretrain_AMASS=False, continue_train=False):
        ...
        # gt_trainset = cnet_pose_dataset(data_train, transform=train_transform, use_feats=self.config.use_features)
        ...
        gt_valset = cnet_pose_dataset(data_val, datasets=self.config.trainsets, 
                                      backbones=self.config.train_backbones, 
                                      config=self.config, train=True)
        ...
        if pretrain_AMASS:
            ckpt_name = 'pretrain' + self.config.ckpt_name
        else:
        ...
        for ep in tqdm(range(eps), dynamic_ncols=True):
            ...
            for batch_idx, data in enumerate(gt_trainloader):
                ...
                if self.config.use_features:
                    kpt_inp = backbone_pred[:,self.in_kpts].flatten(1)
                    fm_inp = feats.flatten(1)
                    inp = [kpt_inp, fm_inp]
                else:
